...
private[ui] class JobPage(parent: JobsTab, store: AppStatusStore) extends WebUIPage("job") {
  ...
  def render(request: HttpServletRequest): Seq[Node] = {
    ...
    val activeStagesTable =
      new StageTableBase(store, request, activeStages.toSeq,
        "active", "activeStage", parent.basePath,
    ...
    val completedStagesTable =
      new StageTableBase(store, request, completedStages.toSeq,
        "completed", "completedStage",
        parent.basePath, basePath, parent.isFairScheduler,
        killEnabled = false, isFailedStage = false)
    val failedStagesTable =
      new StageTableBase(store, request, failedStages.toSeq,
        "failed", "failedStage", parent.basePath,
        basePath, parent.isFairScheduler,
